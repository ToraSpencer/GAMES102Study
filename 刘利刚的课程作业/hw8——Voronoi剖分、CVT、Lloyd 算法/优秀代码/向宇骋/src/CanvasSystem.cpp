		...
		if (ImGui::Begin("Canvas")) {
			...
			if (data->sampling_points.size() >= 6) {
				...
				for (auto siter = data->sampling_points.begin(); siter != data->sampling_points.end(); siter++) {
					...
					// 求当前采样点(*siter)对应的voronoi多边形区域，将多边形顶点（含矩形4顶点）保存在voronoi_polygon中，并作Lloyd Relaxation
					...
					do {
						...
						if (ec->has_source())
							...
							// 当半边(*ec)为射线时，先将vd转换成DT格式，再用dt.dual(ec->dual())求得Ray_2格式射线ray
							// 参考http://cgal-discuss.949826.n4.nabble.com/Voronoi-unbounded-halfedge-to-ray-2-td4662076.html 第5楼
						...
						// 存储位于区域内的source_point
						if (isSpInside)
							voronoi_polygon.push_back(sp);

						// 存储voronoi边与矩形边界相交的点，以及矩形4顶点
...
bool IsPointInPolygon(float x, float y, const std::vector<Ubpa::pointf2>& P) {
	...
	float linePoint1x = x;
	float linePoint1y = y;
	float linePoint2x = minX - 10.0f; // 取最小的X值还小的值作为射线的终点
	float linePoint2y = y;
